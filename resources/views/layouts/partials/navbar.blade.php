    ...
    <a href="{{ route('home') }}" class="nav-logo" aria-label="Faris Jaya Aluminium home">
        ...
        <div class="nav-logo-text" style="display: flex; align-items: center; gap: 12px;">
            ...
            <div style="display: flex; flex-direction: column; align-items: flex-start; line-height: 1.1;">
                <span style="font-size: 18px; font-weight: 800; letter-spacing: 0.5px;">{{ $ft->navbar['brand_1'] ?? 'FARIS JAYA' }}</span>
                <span
                    style="font-size: 12px; font-weight: 600; color: var(--gold-400); letter-spacing: 2px;">{{ $ft->navbar['brand_2'] ?? 'ALUMINIUM' }}</span>
            </div>
        </div>
    </a>
    <ul class="nav-links">
        <li><a href="{{ Request::is('/') ? '#projects' : route('home') . '#projects' }}">{{ $ft->navbar['menu_1'] ?? 'Portofolio' }}</a></li>
        <li><a href="{{ Request::is('/') ? '#products' : route('home') . '#products' }}">{{ $ft->navbar['menu_2'] ?? 'Produk' }}</a></li>
...
<nav class="mobile-nav" id="mobile-nav" aria-label="Mobile navigation">
    ...
    <a href="{{ route('home') }}#products" class="mobile-link">{{ $ft->navbar['menu_2'] ?? 'Produk' }}</a>
